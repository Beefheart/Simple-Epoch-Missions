...
if(_timeOut > 0)then{ //Mission time out possible
	if(time - _start > _timeOut)then{ //Time is up
		...
		{ sleep 0.1;
			if(isPlayer _x && _x distance _pos < (500 max SEM_AIsniperDamageDistance))exitwith{_playerPresent = true};
...
if(_return < 3)then{
	{
	if(_x isKindOf "Car" || _x isKindOf "tank")then{ _y = _x;
		{if(isPlayer _x && alive _x)exitWith{_return = 3}}count crew _y;
	};
	}forEach _objects;
};
